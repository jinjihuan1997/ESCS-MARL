...
def get_config():
    ...
    Please reach each `render/train/<env>_runner.py` file to find private hyperparameters
    only used in <env>.

    Prepare parameters:
        --algorithm_name <algorithm_name>
            specifiy the algorithm, including `["r_mappo", "mappo", ...]`
        --experiment_name <str>
    ...
    parser.add_argument(
        ...
        help="Which algorithm to train; must match subfolder name in on_policy/ or off_policy/"
    )
    ...
    parser.add_argument("--seed", type=int, default=51, help="Random seed for numpy/torch")
    parser.add_argument(
        "--cuda",
        action="store_false",
        default=True,
    ...
    parser.add_argument(
        "--num_env_steps",
        type=int,
        default=5_000_000,  # int 避免 1e6 作为 float 默认值
    ...
    # env parameters
    parser.add_argument("--env_name", type=str, default="SCEnv", help="specify the name of environment")
    parser.add_argument(
        "--use_obs_instead_of_state",
        action="store_true",
        default=False,
    ...
    parser.add_argument(
        "--use_local_obs",
        default=False,
        help="Whether to use local-only observations (per-agent) instead of global obs.",
    )
    parser.add_argument(
        "--use_centralized_V",
        action="store_false",
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Number of frames to stack as input",
    )
    parser.add_argument(
        "--use_stacked_frames",
        action="store_true",
    ...
    parser.add_argument("--recurrent_N", type=int, default=3, help="The number of recurrent layers.")
    parser.add_argument(
        "--data_chunk_length",
        type=int,
        default=100,
    ...
    parser.add_argument("--lr_bs", type=float, default=3e-4,
                        help="learning rate for the BS actor; if None, fallback to --lr")
    parser.add_argument("--lr_sd", type=float, default=3e-4,
                        help="learning rate for the SD actor; if None, fallback to --lr")
    parser.add_argument(
        "--critic_lr",
        type=float,
        default=3e-4,
    ...
    parser.add_argument(
        ...
        type=float,
        default=1e-5,
        help="RMSprop/Adam epsilon (default: 1e-5)",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="time duration between continuous two model savings.",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=5,
        help="time duration between continuous two log printings.",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=100,
        help="time duration between continuous two evaluation progresses.",
    )
    parser.add_argument(
        "--eval_episodes",
        type=int,

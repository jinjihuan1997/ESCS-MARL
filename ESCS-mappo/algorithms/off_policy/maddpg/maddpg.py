...
import copy
import itertools
from algorithms.offpolicy_utils.util import huber_loss, mse_loss, to_torch
from algorithms.offpolicy_utils.popart import PopArt
from algorithms.off_policy.base.trainer import Trainer
...
class MADDPG(Trainer):
    ...
    def shared_train_policy_on_batch(self, update_policy_id, batch):
        ...
        cent_act, replace_ind_start, cent_nact = self.get_update_info(update_policy_id, obs_batch, act_batch,
                                                                      nobs_batch, navail_act_batch)
        ...
        if self.use_popart:
            target_Qs = rewards + self.args.gamma * (1 - dones_env) * self.value_normalizer[p_id].denormalize(
                next_step_Q)
        ...
        if self.use_per:
            ...
            new_priorities = np.stack([error.abs().cpu().detach().numpy().flatten() for error in errors]).mean(
                axis=0) + self.per_eps
        else:
            if self.use_huber_loss:
    ...
    def cent_train_policy_on_batch(self, update_policy_id, batch):
        ...
        if update_actor:
            ...
            # combine the buffer cent acts with actor cent acts and pass into buffer
            actor_update_cent_acts = mask * actor_cent_acts + (1 - mask) * to_torch(all_agent_cent_act_buffer).to(
                **self.tpdv)

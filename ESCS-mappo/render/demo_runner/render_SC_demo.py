...
from envs.env_wrappers import DummyVecEnv  # 你项目里已有

# ----------------------- 地图参数（固定 3km×3km） ----------------------- #
MAP_SIZE_METERS = 3000.0
GRID_MAJOR = 200.0
SP_VMAX = 300.0  # 颜色条最大值（0..300）
...
def _setup_style():
    ...
    # ---- 关键：选一个支持中文的字体 ----
    # 按平台常见字体给一组候选；取第一个已安装的
    cjk_candidates = [
        "Microsoft YaHei",  # Win 常见
        "SimHei",  # 黑体（Win 常见）
        "Noto Sans CJK SC",  # 谷歌 Noto
        "Source Han Sans SC",  # 思源黑体
        "WenQuanYi Zen Hei",  # Linux 常见
        "Arial Unicode MS",  # 一些系统有
    ]
...
def draw_paper_figure(
    ...
    # sd_start_style -> (marker, facecolor_mode)
    _style_map = {
    ...
    if ds_pos:
        ...
        xs = [p[0] for p in ds_pos]; ys = [p[1] for p in ds_pos]
...
def draw_overview_and_gif(
    ...
    def _draw_frame(fig, gs, upto_slot: int):
        ...
        # 放在轴坐标的某个位置：先选 legend 的对齐角，再给锚点坐标
        LEGEND_LOC = "upper left"  # 例：用图例的左上角去对齐
        LEGEND_ANCHOR = (0.02, 0.98)  # 例：轴坐标 (2% 左边距, 98% 高度)
        ...
        legend_all = ax.legend(
            ...
            bbox_transform=ax.transAxes,  # 明确使用轴坐标系（0~1）
...
def main(argv):
    ...
    parser.add_argument(
        "--model_dir",
        type=str,
        default=r"../results/SCEnv/1actor/Discrete/r_mappo/check/run8/models",

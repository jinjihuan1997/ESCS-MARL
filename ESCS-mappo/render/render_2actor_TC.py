...
改动要点（对齐 EnvCore）：
- 解析 EnvCore 的全量日志（log_full=True）：[RESET] / [STEP] / [SD_HOVER_THEN_FLY] / [Q_STATE] / [SP_REWARD] / [SD_POS]
- DS 颜色 = “当槽位”的 SP（0..300）；从 [SP_REWARD] 的 sp_nz（字符串摘要）恢复为 M 维向量
- 方向与位移与环境完全一致：0° 沿 +x；dx=dist*cos(rad), dy=dist*sin(rad)
- 使用 mpl.colormaps 与 buffer_rgba()；图例用代理句柄显示 Tr-UAV / SD-UAV
- 两栏布局：左主图（轨迹+着色）+ 右信息面板（Top DS by SP、动作摘要）

关键修复：
- 当某槽位没有 [SD_POS] 时，使用 NaN 断线，而不是重复上一位置，避免出现“直线串联远点”的假轨迹。

本版额外修改（按你的两点需求）：
1) 去掉点与点之间的连线，只画更小的点；
2) 图例新增：逐个 SD-UAV 轨迹（含 SD 编号），Tr-UAV 起始点，SD 起始点与终点（以最后一个时刻点为终点）。

【新增（与另一份代码对齐）】
3) 支持加载并叠加卫星/背景图（--bg_image / --bg_alpha / --bg_origin），并在：
   - GIF 每帧
   - 总览 PNG
   - 论文版单图
   三处统一生效。默认地图坐标范围 0..3000 m（3km×3km）；若环境坐标不同，请调整 BG_EXTENT。
"""
...
# ----------------------- 背景图（新增，与另一份代码保持一致） ----------------------- #
DEFAULT_BG_NAME = "figbackground.png"  # 可按需替换为你的底图文件名
    ...
    for slot in range(1, max_slot + 1):
        ...
        for k in k_list:
            ...
            if fly_t > 1e-12:
                ...
                rad = _math.radians(phi_deg)
                dx = dist * _math.cos(rad)  # 0°→ +x
                dy = dist * _math.sin(rad)
...
def _draw_bg(ax, bg_img, bg_alpha: float = 1.0, origin: str = "upper"):
    """将背景图绘制在指定 axes 下方。"""
    if bg_img is None:
        return
...
def _setup_style():
    ...
    if picked is not None:
        ...
        ttf_path = Path(__file__).with_name("fonts") / "simhei.ttf"
        if ttf_path.exists():
...
def _setup_paper_style():
    """
    论文版全局样式（高分辨率、统一字号/线宽；便于 PNG/SVG/EPS 直接投稿）。
    """
    import matplotlib as mpl
    import matplotlib.pyplot as plt
    ...
    mpl.rcParams.update({
        ...
        "pdf.fonttype": 42,  # 便于 Illustrator/Acrobat 编辑
        "ps.fonttype": 42,
...
def draw_paper_figure(
        ...
        # === 新增：非图例标记大小控制（最小改动） ===
        ds_marker_size="auto",  # 数值=固定像素；"auto"/None=自适应
        sd_start_size: int = 24,  # SD 起点
        sd_end_size: int = 26,  # SD 终点
        tr_start_size: int = 120,  # Tr-UAV 起点（scatter 的 s 参数）
        size_scale: float = 1.0,  # 对上述所有“非图例”标记做整体缩放
        # === 背景图参数（新增） ===
        bg_img=None,
        bg_alpha: float = 0.85,
        bg_origin: str = "upper",
    ...
    for k, pts in sorted(paths.items(), key=lambda kv: kv[0]):
        ...
        if _np.any(finite):
            ax.scatter(xs[finite], ys[finite],
                       s=TRAJ_PT_SIZE, color=col, edgecolors="white",
                       linewidths=0.35, alpha=0.95, zorder=4, rasterized=False)
    ...
    try:
        ...
        if _np.isfinite(tx) and _np.isfinite(ty):
            ...
            ax.scatter([tx], [ty], marker="^", s=TR_SIZE,
                       ...
                       c="#4682B4", alpha=0.98, zorder=8)
            has_tr = True
    except Exception:
        has_tr = False

    # Legend（保持与原版一致，独立于 size_scale）
    if show_legend:
        start_handle = Line2D([0], [0], marker=_marker, linestyle='None',
                              markerfacecolor='#808080', markeredgecolor='black',
                              markeredgewidth=0.9, markersize=6, label='SD start')
        handles = [
            ...
            Line2D([0], [0], marker='X', linestyle='None',
                   ...
                   markersize=6, label='SD end'),
        ]
        if has_tr:
            handles.append(
                Line2D([0], [0], marker='^', linestyle='None',
                       markerfacecolor='#4682B4', markeredgecolor='white',
                       markeredgewidth=1.0, markersize=7, label='Tr-UAV start')
            )
        for i in range(int(K or 0)):
            handles.append(
                Line2D([0], [0], marker='o', linestyle='None',
                       ...
                       markersize=5, label=f'SD-{i} trajectory')
            )
...
def draw_overview_and_gif(
    ...
    def _draw_frame(fig, gs, upto_slot: int):
        ...
        if ds_pos:
            ...
            cbar.set_label(f'SP', rotation=90, fontsize=11)
        ...
        for k, pts in sorted(paths.items(), key=lambda kv: kv[0]):
            ...
            if np.any(finite):
                # 轨迹点（不连线）
                ax.scatter(xs[finite], ys[finite], s=TRAJ_PT_SIZE, color=col,
                           edgecolors="white", linewidths=0.4, alpha=0.95, zorder=4)
                # 起点 = 第一个有限值（空心圆）
        ...
        example_handles = [
            ...
            Line2D([0], [0], marker='o', linestyle='None',
                   ...
                   markeredgewidth=0.6, markersize=6, label='SD 轨迹点'),
            # SD 起点 / 终点
            Line2D([0], [0], marker='o', linestyle='None',
                   markerfacecolor='none', markeredgecolor='black',
                   markeredgewidth=1.0, markersize=7, label='SD 起始点'),
            Line2D([0], [0], marker='X', linestyle='None',
                   markerfacecolor='black', markeredgecolor='black',
                   markersize=7, label='SD 终点'),
            # Tr-UAV 起始点
            Line2D([0], [0], marker='^', linestyle='None',
                   markerfacecolor='#4682B4', markeredgecolor='white',
                   markeredgewidth=1.2, markersize=10, label='Tr-UAV location'),
        ]

        # 各 SD 的颜色对照（同一图例盒中）
        K_show = K if K is not None else K_infer
        color_handles = [
            Line2D([0], [0], marker='o', linestyle='None',
                   markerfacecolor=soft_colors[i % len(soft_colors)],
                   markeredgecolor='white', markeredgewidth=0.6,
                   markersize=6, label=f'SD-{i}')
            for i in range(int(K_show or 0))
        ]

        LEGEND_LOC = "upper left"
        LEGEND_ANCHOR = (0.02, 0.98)

        legend_all = ax.legend(
            handles=(example_handles + color_handles),
            title='示例与颜色说明',
            loc=LEGEND_LOC,
            bbox_to_anchor=LEGEND_ANCHOR,
            bbox_transform=ax.transAxes,
            framealpha=1.0, fancybox=True, shadow=False,
            fontsize=9, borderpad=0.6, title_fontsize=10,
            ncol=(2 if (K_show and K_show > 6) else 1)
        )
        legend_all.get_frame().set_facecolor('#D9D9D9')
        legend_all.get_frame().set_edgecolor('#8C8C8C')

        # —— 右栏信息 —— #
        M_show = M if M is not None else M_infer
        spv = _sp_vec_for_slot(upto_slot, M_show or 0)
        ...
        info_lines.append(f"[AGENTS] SD-UAV: {K_show}   DS: {M_show}")
        if spv.size > 0:
    ...
    with imageio.get_writer(gif_path, mode="I", duration=1.0 / max(1, int(fps)), loop=0) as writer:
...
def _save_timeseries_csv(out_dir: Path, base: str, slots, values):
    """保存为 out_dir/base.csv，列：slot, base"""
    out_dir.mkdir(parents=True, exist_ok=True)
...
def build_avg_rx_series(parsed: dict, reduce: str = "mean"):
    """
    返回：slots(1D int)、rx(1D float)
    rx 为每槽 DS_rx 的归约：默认 "mean"（对 M 个 DS 的平均），如需总接收量可设为 "sum"。
    """
    import numpy as _np
    ...
    rx = _np.asarray(vals, dtype=float)
    return slots, rx
# ==== END NEW ====
...
def main(argv):
    ...
    parser.add_argument(
        "--model_dir",
        type=str,
        default=r"../results/TCEnv/2actor/Discrete/r_mappo/check/run2/models",
    ...
    # ==== NEW: 构建“时间槽 T”为横轴的两条序列 ====
    sp_slots, sp_vals = build_avg_sp_series(parsed)
    rx_slots, rx_vals = build_avg_rx_series(parsed, reduce="mean")  # 改为总接收量用 reduce="sum"

    # 保存 CSV（列： slot, value ；文件名即列名）
    if sp_slots.size:
    ...
    # >>> 生成论文版单图（无标题、无右栏、无连接框高亮）
    paper_png_path = draw_paper_figure(
        out_dir=out_dir, parsed=parsed, paths=paths,
        figsize=(6, 6), show_legend=True,
        # — 非图例大小控制 —
        ds_marker_size=20,  # 固定 DS 点大小；不想固定就用 "auto"
        traj_pt_size=7,  # 轨迹点更细
        sd_start_size=60,  # SD 起点稍小
        sd_end_size=45,  # SD 终点稍小
        tr_start_size=90,  # Tr-UAV 起点稍小
        size_scale=1.0,  # 一键整体缩放（不影响图例）
        # — 图例大小（你原先已有） —
        legend_fontsize=7,
    ...
    print(f">>> 论文版总览图：{paper_png_path}")
    print(f">>> 渲染完成！GIF: {gif_path}")
    print(f">>> 总览图   ： {png_path}")
